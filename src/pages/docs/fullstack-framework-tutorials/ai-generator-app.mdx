...
The first plan is called "Free" and has the key "plan:free@0". It has two features: "aicopy" and "base". The "aicopy" feature has a mode set to "volume" and a single tier with an "upto" value of 1. The "base" feature has a value of 0. At the high level this means that a customer is only upto 1 of the base "aicopy" generation similar to a free tier popular with products that seek to increase adoption by getting more users to test out the app.

The second plan is called "Startup" and has the key "plan:startup@0". It has three features: "aicopy", "base", and "extraaicopy". The "aicopy" feature has a mode set to "volume" and a single tier with an "upto" value of 4. The "base" feature has a value of 2000. The "extraaicopy" feature has a title set to "AI Generated Extra - Copy", a mode set to "volume", and a single tier with a "price" value of 500. Conceptually to understand this pricing plan by putting all of the above we have a base price of $20.00 that allow the user upto 4 "aicopy" generated results included in their monthly subscription and additional copies ("extraaicopy") are billed to the users at $5.00 per copy.

The third plan is called "Business" and has the key "plan:business@0". It has three features: "aicopy", "base", and "extraaicopy". The "aicopy" feature has a mode set to "volume" and a single tier with an "upto" value of 10 and a "base" value of 0. The "base" feature has a value of 4000. The "extraaicopy" feature has a mode set to "volume" and a single tier with a "price" value of 400. Similarly, to the startup plan, this plan the price per copy is reduced to $4.00 for each additional copy but each plan now includes 10 generated copies.
...
The main way to make the changes would to open the frontend would be to edit this react Generate component in this `src/components/app/GenerateSection.tsx`. Here is a breakdown of the component, so you can feel confident in editing it:
...
To change the backend - open `src/app/api/generate/route.ts` and change the prompt to this:
